...
.factory('MtpApiManager', function (Storage, MtpAuthorizer, MtpNetworkerFactory, MtpSingleInstanceService, ErrorService, qSync, $q, SplayChatMeWebService) {
  var cachedNetworkers = {},
      cachedUploadNetworkers = {},
      cachedExportPromise = {},
      baseDcID = false;

  var SplayChatMeNotified;
  ...
  function SplayChatMeNotify (newValue) {
    if (SplayChatMeNotified !== newValue) {
      SplayChatMeNotified = newValue;
      SplayChatMeWebService.setAuthorized(SplayChatMeNotified);
    }
  }
  ...
  function mtpGetUserID () {
    return Storage.get('user_auth').then(function (auth) {
      SplayChatMeNotify(auth && auth.id > 0 || false);
      return auth.id || 0;
    });
  }
