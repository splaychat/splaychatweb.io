...
gulp.task('compress-dist', ['build'], function() {
  return gulp.src('**/*', {cwd:  path.join(process.cwd(), '/dist')})
    .pipe($.zip('web-ndeipi_v' + pj.version + '.zip'))
...
gulp.task('update-version-comments', function() {
 return gulp.src('app/**/*.js')
  .pipe($.replace(/web-ndeipi v[0-9.]*/, 'web-ndeipi v' +  pj.version))
  .pipe(gulp.dest('app'));
});
  ...
  return es.concat(
    gulp.src(sources)
      .pipe($.manifest({
          timestamp: true,
          network: ['http://*', 'https://*', '*'],
          filename: 'web-ndeipi.appcache',
          exclude: ['web-ndeipi.appcache', 'app.manifest']
        })
      )
      .pipe(gulp.dest('./dist')),
